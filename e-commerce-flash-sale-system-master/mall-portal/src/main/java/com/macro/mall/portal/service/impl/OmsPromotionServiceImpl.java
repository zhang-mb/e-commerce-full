...
public class OmsPromotionServiceImpl implements OmsPromotionService {
    ...
    public List<CartPromotionItem> calcCartPromotion(List<OmsCartItem> cartItemList) {
        //1.先根据productId对CartItem进行分组，以spu为单位进行计算优�?        Map<Long, List<OmsCartItem>> productCartMap = groupCartItemBySpu(cartItemList);
        //2.查询所有商品的优惠相关信息
        ...
        for (Map.Entry<Long, List<OmsCartItem>> entry : productCartMap.entrySet()) {
            ...
            if (promotionType == 1) {
                ...
                for (OmsCartItem item : itemList) {
                    ...
                    //商品原价-促销�?                    PmsSkuStock skuStock = getOriginalPrice(promotionProduct, item.getProductSkuId());
                ...
                //无优�?                handleNoReduce(cartPromotionItemList, itemList,promotionProduct);
            }
        }
    ...
    private void handleNoReduce(List<CartPromotionItem> cartPromotionItemList, List<OmsCartItem> itemList,PromotionProduct promotionProduct) {
        for (OmsCartItem item : itemList) {
            CartPromotionItem cartPromotionItem = new CartPromotionItem();
            BeanUtils.copyProperties(item,cartPromotionItem);
            cartPromotionItem.setPromotionMessage("无优�?);
    ...
    private PmsProductFullReduction getProductFullReduction(BigDecimal totalAmount,List<PmsProductFullReduction> fullReductionList) {
        //按条件从高到低排�?        fullReductionList.sort(new Comparator<PmsProductFullReduction>() {
            @Override
    ...
    private BigDecimal getCartItemAmount(List<OmsCartItem> itemList, List<PromotionProduct> promotionProductList) {
        ...
        for (OmsCartItem item : itemList) {
            //计算出商品原�?            PromotionProduct promotionProduct = getPromotionProductById(item.getProductId(), promotionProductList);
            PmsSkuStock skuStock = getOriginalPrice(promotionProduct,item.getProductSkuId());
